...
<?php if (empty($inputOpts['disabled']) || ($isAdmin && $showFormButtons)) { ?>
    <div class="pull-right">
        <?= Html::resetButton('<i class="glyphicon glyphicon-repeat"></i> ' . Yii::t('kvtree', 'Reset'), ['class' => 'btn btn-default'])
        ?>
        <?= Html::submitButton('<i class="glyphicon glyphicon-floppy-disk"></i> ' . Yii::t('kvtree', 'Save'), ['class' => 'btn btn-primary'])
        ?>
        <?php
        if(!$node->isNewRecord){
            echo Html::a('<i class="fa fa-camera"></i>', Url::toRoute(['/media', 'model' => StringHelper::basename($node->className()), 'modelId' => $node->id, 'iframe' => true]), [
                            'title' => Yii::t('app', 'Media'),
                            'class' => 'btn btn-primary colorboxIframe',
                            //'data-pjax' => '0',
                ]);
        }
        ?>
...
<?php } ?>
<h3><?php echo $parentName . $name ?></h3>
