...
class JQUploadHandler {
    ...
    protected $error_messages = array(
        ...
        'db_save' => "Can't save file to database",
        'db_delete' => "Can't delete file from database",
    );
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            'model' => '',
            'model_id' => null,
            'script_url' => Url::toRoute('/media/jqupload'),
            'upload_dir' => Yii::getAlias('@uploadPath') . '/',
            'upload_url' => Url::to('@uploadUrl') . '/',
            'user_dirs' => date('Y/m/d'),
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|pdf|doc|docx|txt|mp3|mp4)$/i',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null, $extension = null) {
        $file = new \stdClass();
        $file->name = $this->get_file_name($uploaded_file, $name, $size, $type, $error, $index, $content_range);
        //if($type == 'application/pdf'){
        if(!preg_match('/\.(gif|jpe?g|png)$/', $file->name)){
...
//                )) : null;
        $file_name = Yii::$app->security->generateRandomString(16);
        ...
        if ($upload) {
            if (is_array($upload['tmp_name'])) {
                ...
                foreach ($upload['tmp_name'] as $index => $value) {
                    $extension = end(explode('.', $upload['name'][$index]));
                    $files[] = $this->handle_file_upload(
                            $upload['tmp_name'][$index], $file_name ? $file_name : $upload['name'][$index], $size ? $size : $upload['size'][$index], $upload['type'][$index], $upload['error'][$index], $index, $content_range, $extension
                    );
                }
            } else {
                ...
                $files[] = $this->handle_file_upload(
                        isset($upload['tmp_name']) ? $upload['tmp_name'] : null, $file_name ? $file_name : (isset($upload['name']) ?
                                ...
                                $upload['type'] : $this->get_server_var('CONTENT_TYPE'), isset($upload['error']) ? $upload['error'] : null, null, $content_range, $extension
                );
            }
        }
        $response = array('files' => $files); //array($this->options['param_name'] => $files);
        return $this->generate_response($response, $print_response);
    }
     ...
     * @param obj $file
     */
    protected function dbSave($file) {
        $oMedia = new Media();
        $oMedia->filename = $file->name;
        $oMedia->size = $file->size;
        $oMedia->mime = $file->type;
        $oMedia->path = Yii::getAlias('@uploadUrl') . '/' . $this->options['user_dirs'] . '/';
        $oMedia->model = $this->options['model'];
        $oMedia->model_id = $this->options['model_id'];
        $oMedia->save() or $file->error = $this->get_error_message('db_save'); //\yii\helpers\Html::errorSummary($oMedia);
    }
     ...
     * @param obj $file
     */
    protected function dbDelete($file) {
        Media::deleteAll(['filename' => $file]) or $file->error = $this->get_error_message('db_delete');
    }
